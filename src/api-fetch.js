...
async function getWeatherData(request) {
    ...
    if (response.ok) {
      ...
      for (let i = 1; i < 7; i++) {
        ...
        returnData[weatherData.days[i].datetime]['icon'] = weatherData.days[0].icon;
        returnData[weatherData.days[i].datetime]['temp'] = weatherData.days[0].temp;
      }
      return returnData
    } else {
